...
@Component({
  selector: 'app-demographictestassumption',
  templateUrl: './demographictestassumption.component.html',
  styleUrls: ['./demographictestassumption.component.css']
})
export class DemographictestassumptionComponent implements OnInit {
...
this.lgModal4.hide();
  ...
  getassumption() {
    let effect = "INCREASES";
    this._APIwithActionService.getDatabyID(this.forecastid, 'MMProgram', 'GetDemographicMMGeneralAssumptions', "param=" + this.programid + ",3").subscribe((resp) => {
  ...
  save()
  {
    this._APIwithActionService.postAPI(this.selectedassumption, "MMProgram", "saveDemographicMMGeneralAssumptions").subscribe((data) => {
      ...
      this._router.navigate(["/Forecast/Demographicproductassumption", this.forecastid,this.programid]);  
